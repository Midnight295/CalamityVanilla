    ...
    public class ParasiticBeltImbue : ModPlayer
    {
        ...
        public override void MeleeEffects(Item item, Rectangle hitbox)
        {
            if (beltIchorImbue && item.DamageType.CountsAsClass<MeleeDamageClass>() && !item.noMelee && !item.noUseGraphic)
            {
                ...
                {
                    Dust dust = Dust.NewDustDirect(new Vector2(hitbox.X, hitbox.Y), hitbox.Width, hitbox.Height, DustID.Ichor, Scale: 0.75f);
                    dust.velocity *= 0.4f;
        ...
        public override void EmitEnchantmentVisualsAt(Projectile projectile, Vector2 boxPosition, int boxWidth, int boxHeight)
        {
            if (beltIchorImbue && (projectile.DamageType.CountsAsClass<MeleeDamageClass>() || (projectile.DamageType.CountsAsClass<RangedDamageClass>() || projectile.DamageType.CountsAsClass<MagicDamageClass>() || ProjectileID.Sets.IsAWhip[projectile.type]) && !projectile.noEnchantments))
            {
                ...
                {
                    Dust dust = Dust.NewDustDirect(boxPosition, boxWidth, boxHeight, DustID.Ichor, Scale: 0.6f);
                    dust.velocity *= 0.4f;
